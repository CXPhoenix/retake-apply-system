...
from ..models.course import Course # CourseTimeSlot is part of Course model file
# from ..models.enrollment import Enrollment # Enrollment is not directly used here

# get_now 函式可以保留，以防其他地方使用，但模型中建議使用 datetime.now(timezone.utc)
def get_now(utc_offset: int = 8) -> datetime:
    """獲取帶有指定 UTC 時區偏移的當前時間。"""
    return datetime.now(timezone(timedelta(hours=utc_offset)))

def get_utc_now() -> datetime:
    """獲取當前的 UTC 時間 (timezone-aware)。"""
    return datetime.now(timezone.utc)

def format_datetime_to_taipei_str(utc_dt: Optional[datetime], fmt: str = "%Y-%m-%d %H:%M:%S") -> str:
    """
    將 UTC datetime 物件轉換為台北時間 (UTC+8) 的格式化字串。
    如果輸入為 None，則回傳空字串或指定的預設字串。
    """
    if utc_dt is None:
        return "" # 或者您可以定義一個預設值，如 "N/A"
...
def check_course_conflict(
    ...
    此檢查基於 current_academic_year。

    衝堂定義 (根據規格文件 2.4):
    1.  時間重疊：不同課程之間，若課程表定的上課時間
        (需考慮日期、週次、星期、節次、上下午時段) 有任何重疊，即視為衝堂。
        (此部分由 CourseTimeSlot.overlaps_with 處理)
    ...
    參數:
        ...
        current_academic_year (str): 當前的學年度，用於確認比較範圍。

    返回:
        Optional[str]: 如果有衝堂，返回衝突原因的描述字串；若無衝堂，返回 None。
    """
    if selected_course.academic_year != current_academic_year:
        # 理論上 selected_course 應該已經是當前學年的課程，但多一層防護
        return f"選課錯誤：課程 '{selected_course.course_name}' ({selected_course.course_code}) 不屬於當前學年 {current_academic_year}。"
    ...
    for enrolled_course in enrolled_courses:
        ...
        # 檢查定義二: 同課程不同時段
        # 如果已選課程中，有與欲選課程的 course_code 和 academic_year 都相同的，則表示衝堂。
        if enrolled_course.course_code == selected_course.course_code:
            # 由於 selected_course 和 enrolled_course 的 academic_year 都是 current_academic_year
            # 所以這裡隱含了 academic_year 的比較
            return f"課程衝突：您已選修過 '{selected_course.course_name}' ({selected_course.course_code}) 的其他時段。"

        # 檢查定義一: 時間重疊 (不同課程之間)
        # 遍歷欲選課程的每一個 time_slot
        for selected_slot in selected_course.time_slots:
            # 遍歷已選課程的每一個 time_slot
